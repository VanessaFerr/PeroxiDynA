...
resetThreshold;
waitForUser("Threshold POs", "Select & Apply");
close("Threshold");
run("Watershed");
run("Convert to Mask");
selectWindow("nuc");
...
setOption("BlackBackground", true);
resetThreshold;
waitForUser("Threshold Cytoplasm", "Select & Apply");
setTool("freehand");
waitForUser("Cell selection", "outline the cell");

// Save selection in ROI manager
roiManager("reset");
roiManager("Add");
roiManager("Select", 0);
roiManager("Rename", "Cell_ROI");
...
filename = getString("Enter a name for the table (no extension)", "Results_POs");
...
run("Measure");
selectWindow("Results");
filename = getString("Enter a name for the table (no extension)", "Results_cell");
fullPath = baseDir + filename + ".csv";
saveAs("Results", fullPath);
close("Results");
selectWindow("2-cyt");
...
selectWindow("Results");
filename = getString("Enter a name for the table (no extension)", "Results_distancePOs");
fullPath = baseDir + filename + ".csv";
saveAs("Results", fullPath);
close("Results");
imageCalculator("Subtract create", "EDM of 3-nuc","2-cyt");
selectWindow("Result of EDM of 3-nuc");
roiManager("Select", "Cell_ROI");
run("Clear Outside");
run("Set Measurements...", "area min redirect=None decimal=3");
run("Measure");
selectWindow("Results");
filename = getString("Enter a name for the table (no extension)", "Results_distanceMAX");
fullPath = baseDir + filename + ".csv";
saveAs("Results", fullPath);
close("Results");
run("Images to Stack", "use keep");
